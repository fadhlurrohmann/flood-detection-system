...
· RS485 Anemometer · A7670E or SIM7600 (auto-detect, ONLY wrong satu dipasang)
· Relay 5V · Sirine 12V**

> There is no flame sensor di hardware ini. There is no buzzer separate — cukup
> satu relay + sirine (2 tingkat eskalasi through pola berdenyut vs nyala terus,
> see `alarm/siren.py`). Threshold & keputusan alarm level dievaluasi local
> ONLY for menyalakan sirine real-time — NOT disimpan ke local database,
...
| Fungsi | GPIO (BCM) | Pin Physical | Keterangan |
|--------|-----------|-----------|------------|
...
| 5V Rail | — | Pin 2 & 4 | Power LLC HV (jangan from sini If current besar) |
...
BME280 **NOT** through MCP3008/LLC — module ini I2C native, directly ke Pi.
...
| CH0-CH5 | see tabel channel di bawah | All through LLC |
...
All signal analog 0-5V **wajib** through LLC before enter MCP3008 (VREF 3.3V).
...
> Heater ~150mA — power directly from buck converter, jangan from Pi GPIO 5V.
...
> Kalibrasi wajib per probe (see `sensors/soil.py`): dry_raw di udara dry, wet_raw terendam air.

### Submersible Pressure Sensor — loop 4-20mA (Water level)

Sensor ini **loop-powered 2-kabel** (not 0-5V directly), jadi wiring-nya beda
from sensor lain: needs **burden resistor** presisi for mengubah loop current
menjadi voltage that can dibaca ADC.
...
| Loop exit (After sensor) | Ujung atas R_BURDEN (250Ω, 0.1%) |
| Ujung bawah R_BURDEN | GND bersama & PSU (−) |
| Titik sambung sensor/R_BURDEN | LLC **HV-5** → LV-5 → MCP3008 **CH4** |

**Kenapa 250Ω persis?**
- 4mA × 250Ω = **1.0V** → level "empty" (0m)
- 20mA × 250Ω = **5.0V** → level "full" (`PRESSURE_RANGE_M`, default 5m — sesuaikan datasheet sensor Anda)

Formula konversi ada di `sensors/pressure.py`. **Sesuaikan** `EFWS_PRESSURE_RANGE_M`
di `.env` with rentang kedalaman/pressure physical sensor Anda (banyak varian: 0-5m,
0-10m, 0-20m). Payload API sending dua value from sensor ini: `waterLevel` (meter)
and `waterLevelCurrentMa` (loop current raw, berguna buat backend mendeteksi loop
disconnected — value mendadak jatuh ke ~0mA berarti kabel disconnected, not air empty).

### Module Sensor Voltage DC 0-25V (battery)

Module ini already punya voltage divider internal (NOT need buat sendiri).

| Pin module | Hubung ke |
|-----------|-----------|
...
`BATTERY_MIN_V` di `.env` sesuai specification battery Anda (default 12.6V full,
...
### A7670E or SIM7600 (pilih wrong satu)

NOT need wiring berbeda antara keduanya — **ONLY pasang wrong satu module**,
...
| connection | Detail |
|---------|--------|
| Power | Sesuai board HAT (biasanya 5V from Pi or 3.7-4.2V Li-ion separate) |
| Data | USB ke Pi — appear sebagai several `/dev/ttyUSBx` |
| Antenna LTE | Wajib |
| Antenna GNSS | Wajib separate |
| SIM card | Pasang before power-on |

```bash
ls /dev/ttyUSB*
python3 tests/test_sim_detector.py   # konfirmasi module mana that detected
```
...
> There is no buzzer separate — satu relay ini menangani 2 tingkat eskalasi
> (WARNING = berdenyut pelan, CRITICAL = nyala terus), see `alarm/siren.py`.

---

## 6. Diagram Blok Signal Lengkap
...
A7670E / SIM7600 (USB) ─────────┤
                       ...
                       3) evaluasi local → sirine (real-time, NOT disimpan)
                       4) coba send ke API — failed? enter queue (api_queue)
...
## 7. Catu power Every Beban

| Beban | Voltage | source | Catatan |
|-------|---------|--------|---------|
...
| MQ-2 / MQ-135 heater | 5V | Buck converter directly | ~150mA masing-masing |
...
| Module sensor voltage (battery) | Pasif, tap from Battery+/− | — | NOT need suplai separate |
...
[ ] Common ground: Pi, MCP3008, LLC, all sensor, relay, PSU pressure sensor → satu GND
[ ] LLC: HV=5V, LV=3.3V, 6 channel from sensor connected (CH0-CH5)
[ ] MCP3008 VDD & VREF ke 3.3V (not 5V)
[ ] R_BURDEN 250Ω installed correct di loop pressure sensor, tap ke LLC HV-5
[ ] PSU loop pressure sensor separate from Pi/buck converter 5V
[ ] Module sensor voltage tap directly ke Battery+/− (not through relay)
[ ] Jalur 12V sirine ONLY through relay COM/NO, NOT menyentuh Pi
[ ] ONLY SATU module installed: A7670E or SIM7600 (jangan dua-duanya)
[ ] Antenna LTE + GNSS installed
[ ] SIM card installed before module dinyalakan
