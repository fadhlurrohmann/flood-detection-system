...
class EFWS:
    ...
    def _process_commands(self, commands: list):
        for cmd in commands:
            ...
            if handler is None:
                logger.error("Command '%s' unknown.", command_name)
    ...
    def run(self):
        logger.info(
            "EFWS loop started. Check threshold every: %ds | Routine send (if normal) every: %ds | "
            "Retry queue every: %ds (thread separate)",
